const AccessControl = require('role-acl').AccessControl
...
exports.read = (requester, data) => ac.can(requester.role).context({
...
exports.update = (requester, data) => ac.can(requester.role).context({
    requester: requester.ID,
    owner: data.ID
}).execute('update').sync().on('user')

exports.delete = (requester, data) => ac.can(requester.role).context({
    requester: requester.ID,
    owner: data.ID
